...
class DamBreakSimulation:
    def __init__(self, myid, numprocs, topography_file, finer_zone_path, 
                 sww_continue, case_path, base_resolution, finer_zone_resolution, 
                 tms_dir, radius, gauges):
        if myid == 0:
            print(f"Master: Running with {numprocs} processors.")
        barrier() # ANUGA parallel barrier        
        self.myid = myid
    ...
    def _prepare_master_domain(self):
        ...
        self.domain.set_quantity('elevation', filename=self.topography_file, location='centroids') # Use function for elevation
        self.domain.set_quantity('friction', 0.06, location='centroids')                        # Constant friction 
        self.domain.set_quantity('stage', expression='elevation', location='centroids')         # Dry Bed 
    ...
    def _initialize_local_quantities(self):
        ...
        self.domain.set_boundary({'south':   outflow_BC, # outflow
    ...
    def evolve(self, log_file_path, yieldstep_factor_hr=0.25):
        ...
        try:
            for t in self.domain.evolve(yieldstep=yieldstep_factor_hr * self.Hr, finaltime=self.final_time_seconds):
                ...
                if self.myid == 0:
                    with open(log_file_path, "a") as log:
                        ...
                        log.write(f'time: {t:.2f}s ({t/self.Hr:.3f} Hours, {progress_percent:.1f}%) -- {now.strftime("%d/%m/%y %H:%M")}\n')
                    self.domain.print_timestepping_statistics()
                    print(f"   Current Progress: {t / self.Hr:.3f} Hours")
...
if __name__ == "__main__":
    ...
    senarios_path = '/storage/group/cxs1024/default/mehdi/dam_break/scenario_groups_UQ2'
    ...
    base_resolution = 15_000.0
    finer_zone_resolution = 7_500
    ...
    group_path = os.path.join(senarios_path, group)
    senaio_list = get_subfolders(group_path)

    # Define the log file path in the root directory
    log_file_path = f"logs/_dam_break_simulation_progress_{group}.log"
    ...
    for senario in senaio_list:
        ...
        merged_filename = f'Dam_Break_{group}_{senario}_merged.sww'
        merged_file_path = os.path.join(case_path, merged_filename)

        seed = (int(senario.split("_")[1]) + 1 ) * (int(group.split("_")[1]) + 1)
        dem_perterbed_path = os.path.join(case_path, f'rectified_local_dem_perlinUQ_{seed}.asc')
        topography_file_UQ = add_coherent_noise_to_dem_ascii(
            input_dem_path=topography_file,
            output_dem_path=dem_perterbed_path,
            amplitude_m=0.2,      # ~0.5 m vertical uncertainty
            base_grid=(6, 6),     # sets the *largest* spatial scale
            octaves=5,            # adds smaller scales
            persistence=0.55,
            seed=seed,
            mode="std"
        )
        ...
        try:
            ...
            sww_continue = f'Dam_Break_{group}_{senario}'
            if myid == 0:
            ...
            sim = DamBreakSimulation(
                myid, numprocs, topography_file_UQ, finer_zone_path,
                sww_continue, case_path, base_resolution, finer_zone_resolution,
            ...
            sim.setup_domain()
            sim.setup_inlets()
            sim.evolve(log_file_path=log_file_path, yieldstep_factor_hr=0.25)
        ...
        if myid == 0:
            ...
            try:
                merge_sww_files_parallel_parts(
                    directory=case_path,
                    output_name=f'Dam_Break_{group}_{senario}_merged.sww',
